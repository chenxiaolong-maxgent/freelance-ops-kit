...
import { ArrowRight, BadgeCheck, Calculator, CalendarRange, Check, ClipboardCheck, FileCheck2, FolderKanban, Gauge, ListChecks, MessageSquareQuote, MessagesSquare, PackageCheck, RadioTower, ReceiptText, Repeat2, ScanSearch, Send, ShieldCheck, Target } from 'lucide-react';
    ...
    <main className="min-h-screen bg-background text-foreground">
        ...
        <div className="grid overflow-hidden rounded-[30px] border border-border bg-card shadow-[0_28px_85px_rgba(24,39,52,0.09)] lg:grid-cols-[1.02fr_0.98fr]">
          <div className="p-6 sm:p-10 lg:p-14">
            <div className="inline-flex items-center gap-2 rounded-full bg-accent/14 px-3 py-1 text-xs font-bold text-accent-foreground">
              <PackageCheck className="size-3.5" /> 15 个完整商品 + 独家项目经营工作流
            </div>
            <h1 className="mt-6 max-w-3xl text-4xl font-semibold tracking-[-0.05em] sm:text-6xl">从找客户到口碑，一套把十五处漏钱点补齐。</h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">从冷启动拓客、需求访谈、产能排期、销售预测、异议处理、报价、客户启动、反馈改稿、主动进度汇报、长期续费、范围与利润管理，到正式验收、尾款到账和客户口碑，一套覆盖自由职业项目完整经营链路的可编辑工具包。</p>

            <ul className="mt-7 grid gap-3 text-sm sm:grid-cols-2">
              <li className="flex items-center gap-2"><Check className="size-4 text-primary" /> 36 份可编辑 Word 模板</li>
            ...
            <div className="mt-9 flex flex-col gap-4 border-t border-border pt-7 sm:flex-row sm:items-center">
              <div><p className="text-xs text-muted-foreground">首批 20 份早鸟价</p><div className="mt-1 flex items-end gap-3"><span className="text-5xl font-semibold tracking-[-0.05em] text-primary">¥69</span><span className="pb-1 text-sm text-muted-foreground line-through">单买合计 ¥435</span></div></div>
          ...
          <div className="ops-grid relative bg-[#18293b] p-6 text-[#f8f2e6] sm:p-9 lg:p-12">
            <div className="relative">
              <div className="mb-6 flex items-center justify-between">
                <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#f3a35c]">Money leak control</p><p className="mt-2 text-sm text-[#b8c5cf]">一条工作流，守住十五个经营节点</p></div>
                <span className="rounded-full border border-white/12 bg-white/6 px-3 py-1 text-xs text-[#d9e1e6]">01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09 → 10 → 11 → 12 → 13 → 14 → 15</span>
              </div>
      ...
      <section id="bundle" className="mx-auto max-w-[1400px] px-5 pb-12 lg:px-8 lg:pb-16">
        <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">What is inside</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">买到的是十五个完整商品，不是阉割版。</h2></div>
          <p className="max-w-md text-sm leading-6 text-muted-foreground">每个子产品都保留原说明、授权、字体或模板文件。组合包额外提供一份把十五套工具串起来的工作流。</p>
        </div>
          ...
          {PRODUCTS.map(({ step, product, detail, icon: Icon }, index) => (
            <article className="rounded-[24px] border border-border bg-card p-6" key={step}>
              ...
              <ul className="mt-5 space-y-2.5 border-t border-border pt-5 text-sm">
                ...
                {index === 1 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 9 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 问卷 · 访谈 · 纪要 · 需求基线</li></>}
                {index === 2 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 6 张工作表 · 13 周容量</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 项目 · 任务 · 超载预警</li></>}
                {index === 3 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 6 张工作表 · 12 个月预测</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 商机 · 跟进 · 冷却预警</li></>}
                {index === 4 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 9 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 异议 · 诊断 · 跟进 · 复盘</li></>}
                {index === 5 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 6 个行业 · 12 页模板</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 付款、验收与合作边界</li></>}
                {index === 6 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 个行业 · 6 页模板</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 新增范围、费用与工期影响</li></>}
                {index === 7 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 9 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 资料 · 会议 · 权限 · 首周</li></>}
                {index === 8 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 9 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 收集 · 冲突 · 台账 · 确认</li></>}
                {index === 9 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 9 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 周报 · 风险 · 决策 · 月度复盘</li></>}
                {index === 10 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 8 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 三档提案 · 范围 · 续费</li></>}
                {index === 11 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 5 张工作表 · 100 行台账</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 经济利润 · 有效时薪 · 变更费</li></>}
                {index === 12 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 9 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 交付 · 验收 · 尾款 · 归档</li></>}
                {index === 13 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 6 张工作表 · 100 行台账</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 8 个指标 · 12 套催款话术</li></>}
                {index === 14 && <><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 3 份 Word · 共 10 页</li><li className="flex gap-2"><Check className="mt-0.5 size-4 text-primary" /> 评价 · 案例 · 转介绍 · 授权</li></>}
              </ul>
            </article>
          ))}
        </div>
      ...
      <section className="border-y border-border bg-[#e9ede3]">
        <div className="mx-auto grid max-w-[1400px] gap-8 px-5 py-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8 lg:py-16">
          <figure className="overflow-hidden rounded-[24px] border border-primary/15 bg-[#18293b] shadow-[0_24px_65px_rgba(26,40,53,0.16)]">
            ...
            <figcaption className="border-t border-white/8 bg-[#18293b] px-5 py-3 text-center text-xs text-[#aebbc5]">完整交付包约 346 MB · 15 个付费商品源文件不会公开下载</figcaption>
          </figure>
          ...
          <div>
            ...
            <p className="mt-4 text-sm leading-6 text-muted-foreground">《项目经营工作流》不是产品目录，而是一条从拓客研究、需求访谈、产能排期、销售预测、异议诊断、底价测算、发报价、项目启动、收口反馈、主动汇报、确认变更、利润复盘、正式验收、追踪回款到索取口碑的操作顺序。</p>
      ...
      <section className="bg-[#18293b] text-[#f8f2e6]">
        <div className="mx-auto grid max-w-[1400px] gap-8 px-5 py-12 lg:grid-cols-[1fr_460px] lg:items-center lg:px-8 lg:py-16">
          <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#f3a35c]">Early bird bundle</p><h2 className="mt-3 max-w-3xl text-3xl font-semibold tracking-[-0.04em] sm:text-5xl">用一顿饭的钱，把十五处漏钱点都补上。</h2><p className="mt-4 max-w-2xl text-sm leading-6 text-[#afbdc7]">ProspectKit PRO ¥29 + BriefPilot PRO ¥29 + CapacityFlow PRO ¥29 + DealBoard PRO ¥29 + ObjectionDeck PRO ¥29 + 报价匠 PRO ¥29 + ScopeGuard PRO ¥19 + KickoffFlow PRO ¥29 + ReviewDock PRO ¥29 + StatusPulse PRO ¥29 + RetainerFit PRO ¥39 + ProfitLens PRO ¥29 + SignoffFlow PRO ¥29 + CashRadar PRO ¥29 + ProofLoop PRO ¥29，单买早鸟合计 ¥435。组合包仍为 ¥69，并额外包含独家工作流。</p></div>
          <div className="rounded-[26px] border border-white/12 bg-white/7 p-6 sm:p-7">
            <div className="flex items-end justify-between"><div><p className="text-sm text-[#c3ced6]">首批 20 份</p><p className="mt-1 text-5xl font-semibold tracking-[-0.05em]">¥69</p></div><span className="rounded-full bg-[#f3a35c] px-3 py-1 text-xs font-bold text-[#18293b]">省 ¥366 + 独家指南</span></div>
